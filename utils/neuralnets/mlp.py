...
class MLP(object):
    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                cache_name = f'bn_{i}'
                beta = params[f'bn_beta_{i}']
                gamma = params[f'bn_gamma_{i}']
        ...
        for i in range(num_layers - 1):
            ...
            ###############################################
            if dropout_config["enabled"]:
                dx = dropout_backward(dx, cache[f'dropout_{j}'])
            ###############################################
            # END OF DROPOUT                              #
            ###############################################
            ...
            ###############################################
            if use_bn:
                dx, dgamma, dbeta = bn_backward(dx,cache[f'bn_{j}'])
                grads[f'bn_gamma_{j}'] = dgamma
                grads[f'bn_beta_{j}'] = dbeta
            ###############################################
            # END OF BATCH NORMALIZATION                  #
            ###############################################  
