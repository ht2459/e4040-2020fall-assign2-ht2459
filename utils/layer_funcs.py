...
def conv2d_forward(x, w, b, pad, stride):
    ...
    filter_height, filter_width,  _, num_of_filters = w.shape
        ...
        for j in range(num_of_filters):
          for height in range(new_height):
            for width in range(new_width):
              out[i, height, width, j] = np.sum(
                x_pad[i, height * stride : height * stride + filter_height, 
                      width * stride : width * stride + filter_width,:]*w[:,:,:,j]) + b[j]
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    batch, height, width, channels = x.shape
    filter_height, filter_width,  _, num_of_filters = w.shape
    _, height_new, width_new, _ = d_top.shape
        ...
        for j in range(0, channels):
          for k in range(filter_height):
            for l in range(0, filter_width):
              dw[k, l, j, i] = np.sum(d_top[:, :, :, i] * x_pad[:, k:k + height_new * stride:stride, l:l + width_new * stride:stride, j])
                        
                    
        
    return dw, d_b, dw.shape
...
def max_pool_forward(x, pool_size, stride):
    ...
    for i in range(batch):
        for j in range(channels):
          for height in range(new_height):
            for width in range(new_width):
              out[i, height, width, j] = np.max(
                x[i, height * stride : height * stride + pool_size, 
                      width * stride : width * stride + pool_size, j])
...
def max_pool_backward(dout, x, pool_size, stride):
    ...
    for i in range(batch):
        for j in range(channels):
          for height in range(new_height):
            for width in range(new_width):
              ind = np.unravel_index(np.argmax(x[i, height*stride:height*stride+pool_size,
                width*stride:width*stride+pool_size, j]), (pool_size, pool_size))
              dx[i, height*stride:height*stride+pool_size, width*stride:width*stride+pool_size, j][ind] = dout[i, height, width, j]
