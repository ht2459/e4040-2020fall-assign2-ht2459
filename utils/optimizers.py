...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            momentums_ = momentums[k] / (1-np.power(beta1, t))
            velocitys_ = velocitys[k] / (1-np.power(beta2, t))
            params[k] += -learning_rate * momentums_ / (np.sqrt(velocitys_) + eps) 
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            params[k] += -learning_rate / (np.sqrt(velocitys_) + eps) * (beta1 * momentums_ + (1 - beta1) * grads[k] / (1 - pow(beta1, t)))
